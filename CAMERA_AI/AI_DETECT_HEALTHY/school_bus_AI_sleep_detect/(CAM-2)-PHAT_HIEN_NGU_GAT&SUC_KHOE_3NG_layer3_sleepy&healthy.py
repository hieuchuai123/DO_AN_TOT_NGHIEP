...
# ========== CONFIG ==========
URL_SNAPSHOT      = "http://192.168.165.75/cam-mid.jpg"  # đổi đúng endpoint
CONNECT_TIMEOUT   = 3.0
READ_TIMEOUT      = 6.0
TARGET_FETCH_FPS  = 6
...
class FaceMeshWorker:
    ...
    def health_score_from_metrics(self, tid, metrics, emotion):
        # Điểm khởi đầu có thể giữ nguyên hoặc tăng nhẹ nếu muốn
        S = 85.0 

        # --- GIẢM MẠNH hình phạt cho PERCLOS ---
        # PERCLOS cao chủ yếu do mắt nhắm khi ngủ. Giảm hệ số nhân xuống thấp.
        S -= min(25.0, 30.0 * max(0.0, metrics["perclos"] - 0.15)) # Giảm cả điểm trừ tối đa

        br = metrics["blink_rpm"]
        # --- GIẢM MẠNH hình phạt cho BLINK RATE THẤP ---
        # Blink rate thấp cũng do mắt nhắm. Giảm hệ số nhân.
        if br < 12:  S -= (12 - br) * 0.5 
        # (Giữ nguyên hoặc giảm nhẹ hình phạt blink rate cao)
        if br > 21:  S -= (br - 21) * 0.8 # Giảm nhẹ từ 1.0

        # --- DUY TRÌ hoặc TĂNG NHẸ hình phạt NGÁP ---
        # Ngáp là dấu hiệu mệt mỏi tốt. Giữ hoặc tăng nhẹ.
        S -= min(25.0, metrics["yawn_rpm"] * 10.0) # Tăng điểm trừ tối đa một chút

        # --- GIẢM MẠNH hình phạt cho ĐẦU NGHIÊNG ---
        # Đầu nghiêng chủ yếu do ngủ gật. Giảm hệ số nhân.
        S -= max(0.0, (0.8 - metrics["stable_pct"]) * 3.0) 

        # --- DUY TRÌ hình phạt EMOTION ---
        # Cảm xúc tiêu cực có thể là dấu hiệu không khỏe.
        emo_pen = {"Sad":10, "Fear":8, "Angry":6, "Surprise":0, "Neutral":0, "Happy":0, "Emotion:OFF":0} 
        S -= emo_pen.get(emotion, 0)

        S = float(max(0.0, min(100.0, S))) # Giữ điểm trong khoảng [0, 100]

        # --- Làm mượt điểm hiển thị ---
        # Có thể làm mượt chậm hơn một chút để điểm ổn định hơn
        self.S_disp[tid] = 0.3 * S + 0.7 * self.S_disp[tid] # Trọng số điểm cũ cao hơn

        # --- Phân loại health ---
        # Xem xét NỚI LỎNG ngưỡng một chút nếu cần, vì điểm S giờ khó giảm hơn
        if self.S_disp[tid] >= 70: health = "Tot"     # Giảm từ 75
        elif self.S_disp[tid] >= 50: health = "On"      # Giảm từ 55
        elif self.S_disp[tid] >= 35: health = "Met moi" # Giảm từ 40
        ...
        while not self.stop:
            ...
            for tid, det in pairs:
                ...
                if tr["ear_filt"] < ear_thr_blink and not self.closed_flag[tid]:
                    ...
                    if 0.01 <= dur <= 0.50: # nới lỏng khoảng thời gian dur để dễ bắt nháy mắt hơn(ban đầu là 0.08<= dur<=0.45)
